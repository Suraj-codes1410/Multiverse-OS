...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (honeypot && typeof honeypot === 'string' && honeypot.trim() !== '') {
      console.warn(
        'SPAM_PROTECTION: Bot detected via honeypot. Simulating successful message submission.'
      );
    ...
    if (!message || typeof message !== 'string' || !message.trim()) {
      return NextResponse.json(
        { error: 'VALIDATION_ERROR', message: 'Message payload is required.' },
        { status: 400 }
      );
    }
    ...
    if (!emailRegex.test(email.trim())) {
      return NextResponse.json(
        { error: 'VALIDATION_ERROR', message: 'Invalid email address format.' },
        { status: 400 }
      );
    }
    ...
    const lastEmailTime = globalAny.contactRateLimits.get(
    ...
    const recipientEmail =
      process.env.CONTACT_EMAIL || 'samantasuraj1410@gmail.com';

    if (!resendApiKey) {
      console.warn(
        'VERCEL_COMPATIBLE: RESEND_API_KEY is not configured. Simulating mail delivery in development mode.'
      );
...
Your message has been received and I will respond as soon as possible.`);
    ...
    if (!ownerEmailRes.ok) {
      ...
      return NextResponse.json(
        {
          error: 'EMAIL_DISPATCH_FAILURE',
          message: 'Failed to deliver message via email service.',
        },
        { status: 502 }
      );
    }
    ...
    try {
      ...
      console.log(
        `Auto-acknowledgement email triggered successfully for ${email}`
      );
    } catch (autoErr) {
      console.warn(
        'Auto-acknowledgement email could not be sent to external recipient:',
        autoErr
      );
    }
