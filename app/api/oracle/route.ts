...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { resolvedQuery, hit: resolvedMemory } =
      conversationalMemoryService.resolve(sessionId, query);
    const queryToUse = resolvedQuery;
    const contextReused =
      conversationalMemoryService.getSession(sessionId).interactions.length > 0;
    ...
    const cacheKey = repositoryName
      ? `${queryToUse.trim()}::repo:${repositoryName}`
      : queryToUse.trim();
    const cachedResponse = queryCacheService.get(cacheKey);
    if (cachedResponse) {
      await conversationalMemoryService.store(
        sessionId,
        queryToUse,
        cachedResponse
      );
      ...
      const cacheEntry = (queryCacheService as any).cacheManager.get(
        queryCacheService.normalize(cacheKey)
      );
    ...
    if (
      narrativeResult.directAnswerAvailable &&
      narrativeResult.directResponse
    ) {
    ...
    if (routeResult.directAnswerAvailable && routeResult.directResponse) {
      ...
      const finalSource =
        routeResult.category === 'Recruiter Insight'
          ? 'recruiter-insight'
          : 'smart-route';
    ...
    const selected = await OracleContextSelector.select(
      queryToUse,
      fullContext
    );
    ...
    const recruiterInsight =
      await RecruiterInsightEngine.evaluateQuery(queryToUse);
    if (recruiterInsight) {
...
`;
    ...
    if (process.env.NODE_ENV !== 'production') {
      ...
      console.log(
        `  Skills (${selected.skills.length}): ${selected.skills.map((s) => s.name).join(', ')}`
      );
      console.log(
        `  Projects (${selected.projects.length}): ${selected.projects.map((p) => p.title).join(', ')}`
      );
      console.log(
        `  Repositories (${selected.repositories.length}): ${selected.repositories.map((r) => r.name).join(', ')}`
      );
      console.log(
        `  Achievements (${selected.achievements.length}): ${selected.achievements.map((a) => a.title).join(', ')}`
      );
      console.log(
        `  Timeline Items: ${selected.timeline.reduce((sum, t) => sum + t.milestones.length, 0)} milestones`
      );
...
---`;
    ...
    // Store in query cache
    const cachedModelUsed =
      response.usage?.modelUsed ||
