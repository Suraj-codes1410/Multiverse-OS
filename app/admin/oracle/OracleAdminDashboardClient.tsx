...
export default function OracleAdminDashboardClient() {
  ...
  const handleClearCache = async () => {
    if (
      !confirm(
        'Are you sure you want to clear the entire query cache and conversational memory sessions?'
      )
    ) {
      return;
    }
    ...
    try {
      ...
      if (res.ok && data.success) {
        setMessage({
          text: 'Cache and conversational memory cleared successfully.',
          type: 'success',
        });
        fetchMetricsAndHealth();
      } else {
      ...
      setMessage({
        text: e.message || 'Error occurred clearing cache.',
        type: 'error',
      });
    } finally {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#0f172a] text-[#f8fafc] font-sans">
        ...
        <p className="text-sm font-medium">
          Loading Oracle Administration Dashboard...
        </p>
      </div>
    );
  }
    ...
    <div className="max-w-[1400px] mx-auto p-8 w-full bg-[#0b0f19] text-[#f1f5f9] font-sans min-h-screen">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-[#1e293b] pb-6 mb-8 gap-4">
        <div className="flex flex-col">
          <h1 className="text-4xl font-extrabold text-[#f8fafc] tracking-tight">
            Oracle Admin
          </h1>
          <span className="text-[#64748b] text-sm mt-1 block">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div className="bg-[#111827] border border-[#1e293b] rounded-xl p-6 shadow-md hover:border-[#3b82f6] transition-all duration-200">
          <div className="flex justify-between items-center mb-5">
            <h3 className="text-lg font-bold text-[#f1f5f9] m-0">
              System Status
            </h3>
            <span
          ...
          <div className="flex flex-col gap-3">
            <div className="flex justify-between items-center text-sm text-[#94a3b8]">
              ...
              <span
                className={`w-2 h-2 rounded-full inline-block ${health?.services.oracle ? 'bg-emerald-500 shadow-[0_0_8px_#10b981]' : 'bg-red-500 shadow-[0_0_8px_#ef4444]'}`}
              />
            </div>
            <div className="flex justify-between items-center text-sm text-[#94a3b8]">
              <span>Sync Service</span>
              <span
                className={`w-2 h-2 rounded-full inline-block ${health?.services.githubSync ? 'bg-emerald-500 shadow-[0_0_8px_#10b981]' : 'bg-red-500 shadow-[0_0_8px_#ef4444]'}`}
              />
            </div>
            <div className="flex justify-between items-center text-sm text-[#94a3b8]">
              <span>Query Cache</span>
              <span
                className={`w-2 h-2 rounded-full inline-block ${health?.services.cache ? 'bg-emerald-500 shadow-[0_0_8px_#10b981]' : 'bg-red-500 shadow-[0_0_8px_#ef4444]'}`}
              />
            </div>
            <div className="flex justify-between items-center text-sm text-[#94a3b8]">
              <span>Memory Storage</span>
              <span
                className={`w-2 h-2 rounded-full inline-block ${health?.services.memory ? 'bg-emerald-500 shadow-[0_0_8px_#10b981]' : 'bg-red-500 shadow-[0_0_8px_#ef4444]'}`}
              />
            </div>
            <div className="flex justify-between items-center text-sm text-[#94a3b8]">
              <span>Analytics Service</span>
              <span
                className={`w-2 h-2 rounded-full inline-block ${health?.services.analytics ? 'bg-emerald-500 shadow-[0_0_8px_#10b981]' : 'bg-red-500 shadow-[0_0_8px_#ef4444]'}`}
              />
            </div>
          </div>
        </div>
        ...
        {/* Oracle Overview */}
        <div className="bg-[#111827] border border-[#1e293b] rounded-xl p-6 shadow-md hover:border-[#3b82f6] transition-all duration-200">
          <h3 className="text-lg font-bold text-[#f1f5f9] mt-0 mb-5">
            Oracle Overview
          </h3>
          <div className="flex flex-col gap-3">
            ...
            <div className="flex justify-between items-center text-sm">
              ...
              <span className="font-semibold text-[#f1f5f9]">
                {metrics?.overview.repositoriesIndexed || 0}
              </span>
            </div>
            <div className="flex justify-between items-center text-sm">
              <span className="text-[#94a3b8]">Projects Indexed</span>
              <span className="font-semibold text-[#f1f5f9]">
                {metrics?.overview.projectsIndexed || 0}
              </span>
            </div>
            <div className="flex justify-between items-center text-sm">
              <span className="text-[#94a3b8]">Knowledge Graph Size</span>
              <span className="font-semibold text-[#f1f5f9] text-right">
                {(metrics?.overview.kgNodesCount || 0) +
                  (metrics?.overview.kgRelationsCount || 0)}
                <span className="text-xs text-[#64748b] block">
                  ({metrics?.overview.kgNodesCount} nodes,{' '}
                  {metrics?.overview.kgRelationsCount} edges)
                </span>
              </span>
            </div>
            <div className="flex justify-between items-center text-sm">
              ...
              <span className="text-xs text-[#cbd5e1] font-semibold">
                {metrics?.overview.lastSync
                  ? new Date(metrics.overview.lastSync).toLocaleString()
                  : 'Never'}
              </span>
            </div>
          </div>
        ...
        <div className="bg-[#111827] border border-[#1e293b] rounded-xl p-6 shadow-md hover:border-[#3b82f6] transition-all duration-200">
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex justify-between items-center text-sm">
              ...
              <span className="font-semibold text-[#f1f5f9]">
                {metrics?.performance.cacheResponseTime || 0} ms
              </span>
            </div>
            <div className="flex justify-between items-center text-sm">
              <span className="text-[#94a3b8]">OpenRouter Avg Latency</span>
              <span className="font-semibold text-[#f1f5f9]">
                {metrics?.performance.openRouterResponseTime || 0} ms
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="flex gap-6 flex-col lg:flex-row">
        ...
        <div className="flex flex-col flex-[2] gap-6">
          ...
          <div className="bg-[#111827] border border-[#1e293b] rounded-xl p-6 shadow-md hover:border-[#3b82f6] transition-all duration-200">
            <h3 className="text-lg font-bold text-[#f1f5f9] mt-0 mb-5">
              AI Provider & Model Failover Telemetry
            </h3>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-5">
              <div className="bg-[#1f2937] border border-[#374151] rounded-lg p-4 text-center flex flex-col justify-center">
                <span className="text-2xl font-extrabold text-[#f8fafc] leading-none mb-1">
                  {metrics?.providers.openRouterCalls || 0}
                </span>
                <span className="text-xs text-[#94a3b8]">API Requests</span>
              </div>
              <div className="bg-[#1f2937] border border-[#374151] rounded-lg p-4 text-center flex flex-col justify-center">
                <span className="text-2xl font-extrabold text-red-500 leading-none mb-1">
                  {metrics?.providers.openRouterFailures || 0}
                </span>
                <span className="text-xs text-[#94a3b8]">Failures</span>
              </div>
              <div className="bg-[#1f2937] border border-[#374151] rounded-lg p-4 text-center flex flex-col justify-center">
                <span className="text-2xl font-extrabold text-amber-500 leading-none mb-1">
                  {metrics?.providers.modelFailovers || 0}
                </span>
              ...
              <div className="bg-[#1f2937] border border-[#374151] rounded-lg p-4 text-center flex flex-col justify-center">
                <span className="text-2xl font-extrabold text-[#f8fafc] leading-none mb-1">
                  {metrics?.providers.providerSuccessRate || 100}%
                </span>
            ...
            <div className="flex justify-between items-center text-sm mb-2">
              ...
              <span className="font-mono text-xs bg-[#1f2937] px-1.5 py-0.5 rounded">
            ...
            <div className="flex justify-between items-center text-sm">
              ...
              <span className="font-semibold text-[#f1f5f9]">
                {metrics?.providers.rateLimit429Events || 0}
              </span>
            </div>
          </div>
          ...
          <div className="bg-[#111827] border border-[#1e293b] rounded-xl p-6 shadow-md hover:border-[#3b82f6] transition-all duration-200">
            <h3 className="text-lg font-bold text-[#f1f5f9] mt-0 mb-5">
              Recruiter & Career Copilot Analytics
            </h3>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
              <div className="bg-[#1f2937] border border-[#374151] rounded-lg p-4 text-center flex flex-col justify-center">
                <span className="text-2xl font-extrabold text-[#f8fafc] leading-none mb-1">
                  {metrics?.recruiter.recruiterQueries || 0}
                </span>
                <span className="text-xs text-[#94a3b8]">
                  Recruiter Queries
                </span>
              ...
              <div className="bg-[#1f2937] border border-[#374151] rounded-lg p-4 text-center flex flex-col justify-center">
                <span className="text-2xl font-extrabold text-[#f8fafc] leading-none mb-1">
                  {metrics?.recruiter.backendEvaluationQueries || 0}
                </span>
                <span className="text-xs text-[#94a3b8]">
                  Backend Evaluations
                </span>
              ...
              <div className="bg-[#1f2937] border border-[#374151] rounded-lg p-4 text-center flex flex-col justify-center">
                <span className="text-2xl font-extrabold text-[#f8fafc] leading-none mb-1">
                  {metrics?.recruiter.aiEvaluationQueries || 0}
                </span>
                <span className="text-[#cbd5e1] text-xs text-[#94a3b8]">
                  AI Evaluations
                </span>
              </div>
              <div className="bg-[#1f2937] border border-[#374151] rounded-lg p-4 text-center flex flex-col justify-center">
                <span className="text-2xl font-extrabold text-[#f8fafc] leading-none mb-1">
                  {metrics?.recruiter.resumeQueries || 0}
                </span>
                <span className="text-xs text-[#94a3b8]">Resume Inquiries</span>
              </div>
              <div className="bg-[#1f2937] border border-[#374151] rounded-lg p-4 text-center flex flex-col justify-center">
                <span className="text-2xl font-extrabold text-[#f8fafc] leading-none mb-1">
                  {metrics?.recruiter.careerCopilotQueries || 0}
                </span>
                <span className="text-xs text-[#94a3b8]">
                  Career Copilot Queries
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-[#111827] border border-[#1e293b] rounded-xl p-6 shadow-md hover:border-[#3b82f6] transition-all duration-200">
            <h3 className="text-lg font-bold text-[#f1f5f9] mt-0 mb-5">
              Recent Query Observability Log
            </h3>
            <div className="overflow-x-auto mt-2">
              <table className="w-full border-collapse text-left text-sm">
                <thead>
                  <tr className="border-b border-[#1e293b]">
                    <th className="px-4 py-3 text-[#94a3b8] font-semibold">
                      Query
                    </th>
                    <th className="px-4 py-3 text-[#94a3b8] font-semibold">
                      Route
                ...
                <tbody>
                  {metrics?.queries.recentQueries &&
                  metrics.queries.recentQueries.length > 0 ? (
                    metrics.queries.recentQueries.map((q, idx) => (
                      <tr
                        key={idx}
                        className="hover:bg-white/[0.02] transition-colors border-b border-[#1e293b]"
                      >
                        <td
                          className="px-4 py-3 text-[#cbd5e1] max-w-[300px] truncate"
                          title={q.query}
                        >
                          {q.query}
                        </td>
                        <td className="px-4 py-3">
                          <span className={getRouteLabelClasses(q.route)}>
                            {q.route}
                          </span>
                        </td>
                        ...
                        <td className="px-4 py-3">
                          <span
                            className={`text-xs font-bold ${q.cacheHit ? 'text-emerald-400' : 'text-[#64748b]'}`}
                          >
                            {q.cacheHit ? 'HIT' : 'MISS'}
                          </span>
                        </td>
                    ...
                    <tr>
                      ...
                      >
                        No query history logs recorded.
                      </td>
                    </tr>
                  )}
                </tbody>
        ...
        <div className="flex flex-col flex-1 gap-6">
          ...
          <div className="bg-[#111827] border border-[#1e293b] rounded-xl p-6 shadow-md hover:border-[#3b82f6] transition-all duration-200">
            ...
            <div className="flex justify-between items-center text-sm mb-2">
              ...
              <span className="font-semibold text-[#f1f5f9]">
                {metrics?.memory.memoryResolutions || 0}
              </span>
            </div>
            <div className="flex justify-between items-center text-sm mb-2">
              <span className="text-[#94a3b8]">Resolved Pronouns</span>
              <span className="font-semibold text-[#f1f5f9]">
                {metrics?.memory.followUpResolutions || 0}
              </span>
            </div>
            <div className="flex justify-between items-center text-sm mb-2">
              <span className="text-[#94a3b8]">Context Reuse Rate</span>
              <span className="text-emerald-400 text-lg font-bold">
                {metrics?.memory.contextReuseRate || 0.0}%
              </span>
            </div>
            <div className="bg-[#1e293b] rounded-full h-1.5 w-full overflow-hidden mb-5">
              <div
                className="bg-emerald-500 h-full rounded-full transition-all duration-300 ease-out"
                style={{ width: `${metrics?.memory.contextReuseRate || 0}%` }}
              ></div>
            </div>

            <h4 className="text-sm font-bold text-[#e2e8f0] uppercase tracking-wider mb-3">
              Recent Memory Operations
            </h4>
            <ul className="list-none p-0 m-0 flex flex-col gap-3">
              {metrics?.memory.recentMemoryEvents &&
              metrics.memory.recentMemoryEvents.length > 0 ? (
                metrics.memory.recentMemoryEvents.slice(-4).map((evt, idx) => (
                  <li
                    key={idx}
                    className="bg-[#1e293b] rounded-md p-3 border-l-4 border-blue-500"
                  >
                    <span className="text-xs text-[#94a3b8] block mb-1">
                      {new Date(evt.timestamp).toLocaleTimeString()}
                    </span>
                    <p className="text-[13px] m-0 text-[#e2e8f0]">
                      Stored query context: "{evt.query.slice(0, 45)}
                      {evt.query.length > 45 ? '...' : ''}"
                      {evt.resolvedMemory && (
                        <span className="bg-[#8b5cf6] text-white text-[10px] px-1 py-0.25 rounded ml-2 font-semibold">
                          Resolved Pronoun
                        </span>
                      )}
                    </p>
                ...
                <li className="text-[#64748b] text-sm text-center py-4">
                  No conversational sessions stored yet.
                </li>
              )}
            </ul>
          </div>
          ...
          <div className="bg-[#111827] border border-[#1e293b] rounded-xl p-6 shadow-md hover:border-[#3b82f6] transition-all duration-200">
            ...
            <div className="flex flex-col gap-2 mb-4">
              ...
              <p className="text-xs text-[#cbd5e1] break-all bg-[#1e293b] p-2 rounded">
                {metrics?.cache.oldestEntry?.query || 'N/A'}
              </p>
            </div>
            <div className="flex flex-col gap-2 mb-6">
              <span className="text-[#94a3b8] text-sm">Newest Entry</span>
              <p className="text-xs text-[#cbd5e1] break-all bg-[#1e293b] p-2 rounded">
                {metrics?.cache.newestEntry?.query || 'N/A'}
              </p>
            </div>
            <h4 className="text-sm font-bold text-[#e2e8f0] uppercase tracking-wider mb-3">
              Most Frequently Hit Cached Queries
            </h4>
            <ul className="list-none p-0 m-0 flex flex-col gap-2">
              {metrics?.cache.mostCachedQueries &&
              metrics.cache.mostCachedQueries.length > 0 ? (
                metrics.cache.mostCachedQueries.map((q, idx) => (
                  <li
                    key={idx}
                    className="text-[13px] text-[#cbd5e1] flex justify-between items-center"
                  >
                    <span className="truncate max-w-[200px]" title={q.query}>
                      "{q.query}"
                    </span>
                    <span className="bg-[#374151] text-[#94a3b8] text-xs px-1.5 py-0.5 rounded flex-shrink-0">
                      {q.count} hits
                    </span>
                  </li>
                ))
