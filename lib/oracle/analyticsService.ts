...
export class AnalyticsService {
    ...
    if (typeof window === 'undefined') {
      try {
        ...
        if (process.env.VERCEL === '1') {
          console.log(
            'VERCEL_COMPATIBLE: Serverless environment detected. Running Analytics in-memory only.'
          );
          return;
        }
  ...
  public recordProviderCall(record: Omit<ProviderRecord, 'timestamp'>): void {
    ...
    console.log(
      ...
      JSON.stringify({
        ...
        isFailover: record.isFailover,
      })
    );
    if (record.isFailover) {
      console.log(
        'FAILOVER_USAGE',
        `Model failover triggered to model: ${record.model}`
      );
    }
  }
  ...
  private logConsoleStats(record: QueryRecord): void {
    ...
    const smartRoutes = this.data.queries.filter(
      (q) => q.route === 'smart-route'
    ).length;
    const smartRouteRate =
      total > 0 ? ((smartRoutes / total) * 100).toFixed(1) : '0.0';
    ...
    console.log(
      'QUERY_LATENCY',
      `Query "${record.query}" took ${record.latencyMs}ms`
    );
    console.log('CACHE_HIT_RATE', `Cache Hit Rate: ${hitRate}%`);
    console.log(
      'SMART_ROUTE_RATE',
      `Smart Route Direct Response Rate: ${smartRouteRate}%`
    );

    if (record.route === 'openrouter') {
      console.log(
        'OPENROUTER_USAGE',
        `OpenRouter API called for query "${record.query}"`
      );
    }
  }
  ...
  public async getDashboardMetrics() {
      ...
      if (cacheManager) {
        ...
        if (cacheMap) {
          ...
          if (entries.length > 0) {
            ...
            oldestEntry = {
              query: sortedEntries[0].query,
              createdAt: sortedEntries[0].createdAt,
            };
            newestEntry = {
              query: sortedEntries[sortedEntries.length - 1].query,
              createdAt: sortedEntries[sortedEntries.length - 1].createdAt,
            };
          }
        }
      }
    ...
    const sortedByLatencyDesc = [...queries].sort(
      (a, b) => b.latencyMs - a.latencyMs
    );
    const slowestQueries = sortedByLatencyDesc
      .slice(0, 5)
      .map((q) => ({ query: q.query, latencyMs: q.latencyMs }));

    const sortedByLatencyAsc = [...queries].sort(
      (a, b) => a.latencyMs - b.latencyMs
    );
    const fastestQueries = sortedByLatencyAsc
      .slice(0, 5)
      .map((q) => ({ query: q.query, latencyMs: q.latencyMs }));
    ...
    const cacheResponseTime =
      cacheQueries.length > 0
        ? parseFloat(
            (
              cacheQueries.reduce((sum, q) => sum + q.latencyMs, 0) /
              cacheQueries.length
            ).toFixed(1)
          )
        : 0;

    const orQueries = queries.filter(
      (q) => q.route === 'openrouter' && !q.cacheHit
    );
    const openRouterResponseTime =
      orQueries.length > 0
        ? parseFloat(
            (
              orQueries.reduce((sum, q) => sum + q.latencyMs, 0) /
              orQueries.length
            ).toFixed(1)
          )
        : 0;
    ...
    const rateLimit429Events = providerCalls.filter(
      (p) => p.errorCode === 429
    ).length;
    ...
    queries.forEach((q) => {
      ...
      if (
        q.route === 'recruiter-insight' ||
        q.category === 'Recruiter Insight'
      ) {
        recruiterQueries++;
      }
      ...
      const hasBackendKw =
        /\b(backend|spring boot|springboot|database|java|go|rust|golang|patient-management-service|microservice|concurrency|distributed|grpc|raft)\b/i.test(
          queryLower
        );
      const hasAiKw =
        /\b(ai|machine learning|vector|rag|pinecone|sahai|orbitair|llm|agents|neural|model)\b/i.test(
          queryLower
        );
      const hasResumeKw =
        /\b(resume|cv|portfolio|interview|hired|hire)\b/i.test(queryLower);
    ...
    try {
      ...
      if (fs.existsSync(cachePath)) {
        ...
        reposIndexed = Array.isArray(cache.repositories)
