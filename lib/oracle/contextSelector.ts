...
export class OracleContextSelector {
  ...
  > {
    ...
    const addSkill = (name: string) => {
      const item = context.skills.find(
    ...
    const addProject = (titleOrId: string) => {
      const item = context.projects.find(
        (p) =>
    ...
    const addRepo = (name: string) => {
      const item = context.repositories.find(
    ...
    const addAchievement = (title: string) => {
      const item = context.achievements.find(
    ...
    const sharedRegex =
      /(?:shared between|connect|commonalities between)\s+([\w\s-]+)\s+and\s+([\w\s-]+)/i;
    const sharedMatch = queryLower.match(sharedRegex);
    if (sharedMatch) {
      const name1 = sharedMatch[1].trim();
      const name2 = sharedMatch[2].trim();

      const sharedTechs = RelationshipDiscoveryService.findSharedTechnologies(
        graph,
        name1,
        name2
      );
    ...
    if (intersectMatch) {
      ...
      const intersectionRepos =
        RelationshipDiscoveryService.findRepositoriesWithAllTech(graph, [
          tech1,
          tech2,
        ]);
    ...
    for (const tech of techList) {
      ...
      if (queryLower.includes(techLower)) {
        const hasKeyword = reverseLookupKeywords.some((kw) =>
          queryLower.includes(kw)
        );
        const startsWithWhich =
          queryLower.startsWith('which') || queryLower.startsWith('what');
    ...
    if (isReverseLookup && targetTech && targetType) {
      ...
      if (targetType === 'Repository') {
        const matchingRepos =
          RelationshipDiscoveryService.findRepositoriesByTech(
            graph,
            targetTech
          );
        ...
        const matchingProjects =
          RelationshipDiscoveryService.findProjectsByTech(graph, targetTech);
    ...
    if (matchedRepo) {
      ...
      const proj = context.projects.find(
        (p) =>
          ...
          (p.githubUrl &&
            p.githubUrl
              .toLowerCase()
              .endsWith('/' + matchedRepo.name.toLowerCase()))
      );
      if (proj) {
        addProject(proj.id);
      }
      ...
      if (matchedRepoByReadme && !matchedRepoByName) {
        const foundWord = queryWords.find((w) =>
          matchedRepo.readmeExcerpt?.toLowerCase().includes(w)
        );
        if (foundWord) {
          const index = queryLower.indexOf(foundWord);
          const originalWord =
            index !== -1
              ? query.substring(index, index + foundWord.length)
              : foundWord;
  ...
  private static resolveEntityNodeId(graph: any, name: string): string | null {
    ...
    for (const node of nodes) {
      if (
        ...
        node.id.split(':').slice(1).join(':').toLowerCase() === lower
      ) {
        return node.id;
      }
    }
   ...
   */
  static compressAndFormat(
    selected: SelectedContext & { repositoryMetadata?: string }
  ): string {
    let output = '';
    ...
    if (selected.repositories.length > 0) {
      ...
      selected.repositories.forEach((r) => {
        ...
        if (r.repositorySummary) {
          ...
          if (
            r.repositorySummary.KeyFeatures &&
            r.repositorySummary.KeyFeatures.length > 0
          ) {
            output += `  Key Features: ${r.repositorySummary.KeyFeatures.join(', ')}\n`;
          }
          if (
            r.repositorySummary.TechnologyStack &&
            r.repositorySummary.TechnologyStack.length > 0
          ) {
            output += `  Technologies: ${r.repositorySummary.TechnologyStack.join(', ')}\n`;
          }
          if (
            r.repositorySummary.ComplexityIndicators &&
            r.repositorySummary.ComplexityIndicators.length > 0
          ) {
