...
export type NarrativeMode =
  'Journey' | 'Growth' | 'Strengths' | 'Career' | 'None';
...
export class NarrativeIntentClassifier {
  public static classify(query: string): NarrativeMode {
    ...
    const hasJourney =
      /\b(journey|chronology|career history|timeline|story|bio|biography|profile)\b/i.test(
        queryLower
      );
    const hasGrowth =
      /\b(evolve|evolved|evolution|grow|grew|growth|progress|progressed|progression|trajectory)\b/i.test(
        queryLower
      );
    const hasStrengths =
      /\b(strength|strengths|abilities|weakness|weaknesses|improve|gaps|assess|assessment)\b/i.test(
        queryLower
      );
    const hasCareer =
      /\b(engineer.*becoming|positioning|specialization|career goal|career path|specialise|specialise)\b/i.test(
        queryLower
      );
    ...
    if (
      queryLower.includes('how did suraj progress') ||
      queryLower.includes('themes in suraj') ||
      queryLower.includes('what type of engineer')
    ) {
      return 'Career';
    }
    ...
    return [
      {
        ...
        description:
          'Foundation building, basic scripting, CRUD interfaces, and initial exposure to languages like Java and Go.',
      },
      {
        stage: 'Intermediate',
        timeframe: '2024',
        ...
        description:
          'Enterprise API development, Spring Boot microservices, messaging patterns (RabbitMQ, Kafka), and advanced relational database tuning.',
      },
      {
        stage: 'Advanced',
        ...
        description:
          'Engineering distributed databases (consensus protocol, gRPC architecture), multi-agent RAG reasoning pipelines, and geospatial timeseries indexing.',
      },
    ];
  }
}
  ...
  public static analyze(
    ...
    projects.forEach((p) => {
      ...
      if (
        ...
        lowerDesc.includes('grpc') ||
        lowerDesc.includes('kafka') ||
...
export class PortfolioNarrativeEngine {
  public static async generate(query: string): Promise<{
    ...
    const needsNarrativeSynthesis = narrativeKeywords.some((kw) =>
    ...
    const { strengths, weaknesses } = StrengthAnalyzer.analyze(
      projects,
      repositories
    );
    ...
    console.log('\nJOURNEY_ANALYSIS');
    console.log(
      `Chronological progression analyzed over ${stages.length} evolutionary stages.`
    );

    console.log('\nSTRENGTH_ANALYSIS');
    console.log(
      `Identified ${strengths.length} core strengths and ${weaknesses.length} development gaps.`
    );

    console.log('\nEVOLUTION_ANALYSIS');
    console.log(
      `Growth path extracted containing ${growthTrajectories.length} trajectory shifts.`
    );
