...
const CATALOG: CatalogItem[] = [
  ...
  { name: 'TensorFlow', category: 'AI / ML', keywords: ['tensorflow'] },
  {
    ...
    category: 'AI / ML',
    keywords: ['machine learning', 'machine-learning', 'ml'],
  },
  {
    name: 'LLM',
    category: 'AI / ML',
    keywords: ['llm', 'large language model', 'openai', 'gemini'],
  },
];
...
export function extractTechnologyProfile(
  ...
  const hasSpecificBackendFramework = backendTech.some((t) =>
    ['FastAPI', 'Spring Boot', 'Django', 'Express', 'NestJS', 'Flask'].includes(
      t
    )
  );
  ...
  if (
    ...
    (topics.includes('nodejs') ||
      topics.includes('node') ||
      textContext.includes('node.js') ||
      textContext.includes('nodejs'))
  ) {
