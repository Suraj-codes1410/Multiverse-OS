...
export function classifyRepository(
  ...
  const textContext =
    `${repo.name} ${repo.description || ''} ${topics.join(' ')} ${language}`.toLowerCase();

  // If intelligence is available, enrich search context
  const intelTechnologies = (intelligence?.technologies || []).map((t) =>
    t.toLowerCase()
  );
  const intelConcepts = (intelligence?.keyConcepts || []).map((c) =>
    c.toLowerCase()
  );
  const intelComplexity = (intelligence?.complexityIndicators || []).map((c) =>
    c.toLowerCase()
  );
  ...
  const hasBackendTopic = topics.some((t) =>
    [
      'backend',
      'spring-boot',
      'fastapi',
  ...
  const hasBackendKeyword = hasKeyword([
    'spring boot',
    'springboot',
    'fastapi',
    'django',
  ...
  if (
    ...
    hasBackendTopic ||
    hasBackendKeyword ||
    intelProjCategory === 'backend engineering'
  ) {
    categories.add('Backend Engineering');
  }

  // 2. Frontend Development
  const isFrontendLanguage = [
    'typescript',
    'javascript',
    'html',
    'css',
  ].includes(language);
  const hasFrontendTopic = topics.some((t) =>
    [
      'react',
      'nextjs',
      'frontend',
  ...
  const hasFrontendKeyword = hasKeyword([
    'react',
    'nextjs',
    'frontend',
    'ui',
  ...
  if (
    ...
    hasFrontendTopic ||
    hasFrontendKeyword ||
    intelProjCategory === 'frontend engineering'
  ) {
    categories.add('Frontend Development');
  }

  // 3. Full Stack Development
  const hasFullStackTopic = topics.some((t) =>
    ['fullstack', 'full-stack', 'website', 'platform'].includes(t)
  );
  const hasFullStackKeyword = hasKeyword([
    'fullstack',
    'full-stack',
    'lifestyle platform',
  ...
  const hasBothFrontAndBack =
    categories.has('Backend Engineering') &&
    categories.has('Frontend Development');
  ...
  if (
    hasAITopic ||
    hasAIKeyword ||
    intelProjCategory === 'ai & data engineering' ||
    repoNameLower === 'orbitair' ||
    repoNameLower === 'sahai'
  ) {
  ...
  // 5. Data Engineering
  const hasDataTopic = topics.some((t) =>
    [
      'data-engineering',
      'pipeline',
      'ingestion',
  ...
  const hasDataKeyword = hasKeyword([
    'pipeline',
    'ingestion',
    'log ingestion',
    'data pipeline',
  ...
  const hasDistributedTopic = topics.some((t) =>
    [
      'distributed',
      'microservices',
      'raft',
  ...
  const hasDistributedKeyword = hasKeyword([
    'distributed',
    'microservices',
    'microservice',
    'raft',
  ...
  if (
    hasDistributedTopic ||
    hasDistributedKeyword ||
    intelProjCategory === 'distributed systems' ||
  ...
  // 7. Developer Tools
  const hasDevToolsTopic = topics.some((t) =>
    [
      'framework',
      'library',
      'cli',
  ...
  const hasDevToolsKeyword = hasKeyword([
    'framework',
    'library',
    'cli',
    'utility',
