...
export const distributedSystemsCommand: Command = {
  ...
  description:
    'Displays a recruiter-focused dossier of Distributed Systems and Microservices engineering experience.',
  execute: async () => {
    ...
    const distProjects = allProjects.filter((p) => {
      ...
      return (
        classifications.includes('Distributed Systems') ||
    ...
    const dsSkills = graph
      .getNodesByType('Skill')
      .filter((s) =>
        dsKeywords.some((kw) => s.label.toLowerCase().includes(kw))
      )
      .map((s) => s.label);
    ...
    distProjects.forEach((p) => {
      ...
      output.push(
        `    - Complexity:    ${p.intelligence?.complexityAnalysis?.overallRating || 'Advanced'}`
      );
      output.push('');
    });
    ...
    distProjects.forEach((p) => {
      p.intelligence?.keyConcepts?.forEach((c) => {
        if (
          c.toLowerCase().includes('raft') ||
          c.toLowerCase().includes('consensus') ||
          c.toLowerCase().includes('event') ||
          c.toLowerCase().includes('rpc') ||
          c.toLowerCase().includes('microservice')
        ) {
          dsConcepts.add(c);
        }
      });
