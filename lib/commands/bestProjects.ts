...
export const bestProjectsCommand: Command = {
  ...
  description:
    'Displays a recruiter-focused dossier of the best featured projects, ranked by verified complexity and awards.',
  execute: async () => {
    ...
    featuredProjects.forEach((p, idx) => {
      const category =
        p.intelligence?.projectType || p.subtitle || 'Software System';
      const rating =
        p.intelligence?.complexityAnalysis?.overallRating || 'Advanced';
