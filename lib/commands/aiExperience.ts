...
export const aiExperienceCommand: Command = {
  ...
  description:
    'Displays a recruiter-focused dossier of Artificial Intelligence and Data Engineering experience.',
  execute: async () => {
    ...
    const aiProjects = allProjects.filter((p) => {
      ...
      return (
        classifications.includes('AI Engineering') ||
    ...
    const aiSkills = graph
      .getNodesByType('Skill')
      .filter(
        (s) =>
          s.properties.category === 'AI / ML' ||
          s.label.toLowerCase() === 'pinecone' ||
          s.label.toLowerCase() === 'timescaledb'
      )
      .map((s) => s.label);
    ...
    aiProjects.forEach((p) => {
      p.intelligence?.keyConcepts?.forEach((c) => {
        if (
          ...
          c.toLowerCase().includes('vector') ||
          c.toLowerCase().includes('spatial') ||
