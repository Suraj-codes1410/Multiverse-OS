...
export const backendExperienceCommand: Command = {
  ...
  description:
    'Displays a recruiter-focused dossier of backend engineering experience, skills, and systems architectures.',
  execute: async () => {
    ...
    const backendProjects = allProjects.filter((p) => {
      ...
      return (
        classifications.includes('Backend Engineering') ||
        p.intelligence?.projectCategory === 'Backend Engineering'
      );
    });

    // 2. Fetch backend skills from Knowledge Graph
    const backendSkills = graph
      .getNodesByType('Skill')
      .filter(
        (s) =>
          s.properties.category === 'Backend' ||
          s.properties.category === 'Database'
      )
      .map((s) => s.label);
