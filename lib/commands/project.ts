...
export const projectCommand: Command = {
  ...
  execute: async (args) => {
    ...
    if (
      inputName === 'patient-management' ||
      inputName === 'patient-management-service' ||
      inputName === 'patientmanagement'
    ) {
    ...
    const project = projects.find(
      (p) =>
        ...
        p.title.toLowerCase().includes(inputName)
    );
    ...
    const openTargetName =
      project.id === 'patient-management-service'
        ? 'patient-management'
        : project.id;
