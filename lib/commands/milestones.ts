...
export const milestonesCommand: Command = {
  ...
  description:
    'Displays academic and career milestones from the Knowledge Graph.',
  execute: async () => {
    ...
    const events = graph
      ...
      .filter(
        (e) =>
          e.properties.category === 'education' ||
          e.properties.category === 'milestone' ||
          e.properties.category === 'achievement'
      );
