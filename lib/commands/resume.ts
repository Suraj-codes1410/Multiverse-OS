    ...
    if (args.length > 0) {
      ...
      if (action === 'view') {
        if (typeof window !== 'undefined') {
          ...
          return {
            output:
              'Server environment detected. Resume action "view" can only run in the browser.',
            success: false,
          };
        }
        ...
        if (typeof window !== 'undefined') {
          ...
          link.href = resumeUrl;
          link.download =
            resumeUrl.substring(resumeUrl.lastIndexOf('/') + 1) || 'resume.pdf';
          ...
          return {
            output:
              'Server environment detected. Resume action "download" can only run in the browser.',
            success: false,
          };
        }
      } else {
