...
export class RelationshipGraphBuilder {
  ...
  public addNode(node: KnowledgeNode): void {
    if (!node.id || !node.type || !node.label) {
      throw new Error(
        `Invalid node definition. Missing required fields: ${JSON.stringify(node)}`
      );
    }
  ...
  public addRelationship(rel: KnowledgeRelationship): void {
    ...
    if (rel.sourceId === rel.targetId) {
      console.warn(
        `Relationship integrity warning: Self-loop detected on ${rel.sourceId}. Skipping.`
      );
      return;
    }
    ...
    if (!sourceNode) {
      console.warn(
        `Relationship integrity warning: Source node ${rel.sourceId} does not exist. Skipping relationship.`
      );
      return;
    }
    ...
    if (!targetNode) {
      ...
      if (rel.targetId.startsWith('skill:')) {
        ...
        console.warn(
          `Relationship integrity warning: Target node ${rel.targetId} does not exist. Skipping relationship.`
        );
        return;
      }
    }
