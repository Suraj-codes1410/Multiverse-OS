...
export function MobileHome() {
  ...
  const apps = [
    { id: 'about', label: 'About Me', icon: User, color: 'text-accent-cyan bg-accent-cyan/8' },
    { id: 'projects', label: 'Projects', icon: Briefcase, color: 'text-accent-purple bg-accent-purple/8' },
    { id: 'skills', label: 'Skills Map', icon: Compass, color: 'text-success-green bg-success-green/8' },
    { id: 'timeline', label: 'Timeline', icon: Calendar, color: 'text-warning-amber bg-warning-amber/8' },
    { id: 'resume', label: 'CV Resume', icon: FileText, color: 'text-accent-cyan bg-accent-cyan/8' },
    { id: 'contact', label: 'Contact', icon: Mail, color: 'text-accent-purple bg-accent-purple/8' },
    { id: 'recruiter', label: 'Recruiter', icon: ShieldAlert, color: 'text-success-green bg-success-green/8' },
    { id: 'explorer', label: 'GitHub Files', icon: Folder, color: 'text-warning-amber bg-warning-amber/8' },
    { id: 'settings', label: 'Settings', icon: Settings, color: 'text-text-secondary bg-bg-panel-hover/40' },
  ];
  ...
  return (
    <div className="flex-1 overflow-y-auto px-6 py-6 flex flex-col gap-6 scrollbar-none">
      ...
      <Card hoverable={false} className="p-4 bg-bg-panel/40 border-border-subtle/35 rounded-2xl shadow-sm flex items-center gap-4.5 select-none relative overflow-hidden">
        {/* SVG Robot Companion */}
        <div className="relative flex-shrink-0 flex items-center justify-center p-1 bg-white border border-window-border rounded-xl shadow-sm">
          <svg width="38" height="38" viewBox="0 0 24 24" fill="none" className="w-10 h-10 animate-bounce-slow">
            {/* Ears / Antennas */}
        ...
        <div className="flex flex-col text-left">
          <span className="text-[10px] font-mono text-accent-cyan uppercase tracking-wider font-bold">OS Companion</span>
          ...
          <p className="text-[10px] text-text-secondary leading-normal mt-0.5 font-light">
            Need details about Suraj's skill matrix? Tap the Oracle shortcut to begin a dialogue.
          </p>
        </div>
      </Card>
      ...
      <section className="flex flex-col gap-3">
        <h3 className="font-mono text-[9px] uppercase tracking-wider text-text-secondary/70">System Applications</h3>
        <div className="grid grid-cols-3 gap-x-4 gap-y-5">
          {apps.map((app) => {
            ...
            return (
              ...
              >
                <div className={`w-13 h-13 rounded-2xl flex items-center justify-center shadow-sm border border-border-subtle/20 transition-all group-hover:shadow-md ${app.color}`}>
      ...
      <section className="flex flex-col gap-3 mb-4 select-none">
        <h3 className="font-mono text-[9px] uppercase tracking-wider text-text-secondary/70">Quick Assistant Queries</h3>
        <div className="flex flex-col gap-2">
          {[
            ...
            <button
              ...
              onClick={() => {
                ...
                if (typeof window !== 'undefined') {
                  const event = new CustomEvent('oracleQuery', { detail: query });
            ...
            >
              <span className="truncate pr-4 leading-normal font-light">{query}</span>
