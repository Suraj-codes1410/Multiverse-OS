  ...
  const technicalAreas = [
    {
      ...
      description:
        'Developing low-latency microservices, concurrent APIs using Go and Rust, and high-performance RPC systems.',
    },
    {
      title: 'Artificial Intelligence',
      icon: <Brain className="w-7 h-7 text-accent-purple" />,
      description:
        'Integrating large language models, setting up multi-agent state machines, and building telemetry-guided prompt loops.',
    },
    {
      title: 'System Design',
      icon: <Cpu className="w-7 h-7 text-success-green" />,
      description:
        'Architecting distributed queues, consensus sharding setups, and managing pipeline backpressures.',
    },
    {
      title: 'Product Development',
      icon: <Layers className="w-7 h-7 text-warning-amber" />,
      description:
        'Creating developer tools and clean system boundaries that combine high-quality code with high reliability.',
    },
  ];

  // If we are on desktop, we do not draw the scrolling website layout in the background.
  // The DesktopShell renders the Wallpaper, Icons, Widgets, and Desktop Windows.
  if (viewportType === 'desktop') {
    return null;
  }

  // Mobile viewport: render original scrolling website homepage layout
  return (
        ...
        <Container>
          <div className="max-w-4xl mx-auto text-center">
            ...
            <p className="text-sm sm:text-lg font-mono text-accent-cyan tracking-wide mb-8">
              Backend Developer{' '}
              <span className="text-text-secondary/40">|</span> AI Engineer{' '}
              <span className="text-text-secondary/40">|</span> Problem Solver
            </p>

            {/* Value Proposition */}
            <h2 className="text-xl sm:text-2xl text-text-secondary max-w-2xl mx-auto leading-relaxed mb-10 font-light">
              Designing high-performance distributed systems and AI solutions
