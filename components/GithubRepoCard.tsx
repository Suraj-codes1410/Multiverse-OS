...
export default function GithubRepoCard({ repo }: GithubRepoCardProps) {
  ...
  return (
    <Card
      hoverable={true}
      className="flex flex-col justify-between h-full p-5 border border-border-subtle bg-bg-panel/40"
    >
      <div>
        {/* Repo Header */}
        <div className="flex items-start justify-between gap-4 mb-2">
          <div className="flex items-center gap-2">
            ...
            <Link
              ...
              className="focus:outline-none group/title"
            >
              <h3
                className="text-sm font-bold text-text-primary group-hover/title:text-accent-cyan transition-colors truncate max-w-[180px]"
                title={repo.name}
              >
                {repo.name}
              </h3>
          ...
          <div className="flex flex-wrap gap-1 mb-2">
            {classifications.map((cat) => (
              <Badge
                ...
                color="purple"
                variant="solid"
                className="text-[8px] py-0.5 px-1.5 uppercase font-bold tracking-wider"
              >
                {cat}
              </Badge>
            ))}
        ...
        <div className="mb-4">
          <Link
            href={`/github/${repo.name}`}
            className="text-[10px] font-mono text-accent-cyan hover:underline inline-flex items-center gap-0.5 select-none focus:outline-none"
          >
        ...
        <div className="mt-3 pt-3 border-t border-border-subtle/20 flex flex-col gap-2">
          ...
          {showReadmeSlot && (
            <div className="p-2 bg-accent-cyan/5 border border-accent-cyan/20 rounded font-mono text-[9px] text-text-secondary leading-normal space-y-1">
              ...
              <p>
                Markdown parsing pipeline ready. Extension point enabled in{' '}
                <code>lib/github/readme.ts</code>. Local cache file slot
                prepared.
              </p>
            </div>
          )}
          ...
          {showIntelligence && (
            <div className="p-2 bg-accent-purple/5 border border-accent-purple/20 rounded font-mono text-[9px] text-text-secondary leading-normal space-y-1">
              ...
              <p>
                Complexity indicators, architecture analysis, and developer
                metrics are available. Open the detail view to see the complete
                scorecard.
              </p>
            </div>
          )}
