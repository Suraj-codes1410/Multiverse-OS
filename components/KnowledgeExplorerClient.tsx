...
export default function KnowledgeExplorerClient({
  ...
  const [selectedNodeId, setSelectedNodeId] = useState<string>(
    initialNodes.find((n) => n.id === 'project:orbitair')?.id ||
      initialNodes[0]?.id ||
      ''
  );
  const [searchQuery, setSearchQuery] = useState('');
  const [activeTypeFilter, setActiveTypeFilter] = useState<NodeType | 'All'>(
    'All'
  );
    ...
    if (searchQuery.trim() !== '') {
      ...
      nodes = nodes.filter((node) => {
        ...
        if (
          ...
          String(node.properties.description).toLowerCase().includes(query)
        )
          return true;
        return false;
      });
  ...
  const getTypeBadgeStyles = (type: NodeType) => {
    switch (type) {
      case 'Project':
        return { color: 'cyan' as const, icon: Cpu };
      case 'Skill':
        return { color: 'purple' as const, icon: Sparkles };
      case 'Repository':
        return { color: 'green' as const, icon: GitBranch };
      case 'Achievement':
        return { color: 'amber' as const, icon: Award };
      case 'Experience':
        return { color: 'default' as const, icon: Briefcase };
      case 'Education':
        return { color: 'default' as const, icon: BookOpen };
      case 'Timeline Event':
        return { color: 'default' as const, icon: Calendar };
      default:
        return { color: 'default' as const, icon: Bookmark };
    }
  };
  ...
  const groupedNeighbors = useMemo(() => {
    const groups: {
      [key in NodeType]?: { node: KnowledgeNode; rel: KnowledgeRelationship }[];
    } = {};
  ...
  return (
    <div className="flex-grow py-8 font-mono text-xs sm:text-sm text-text-secondary select-none">
      <Container>
        ...
        <div className="border border-border-subtle bg-bg-panel/40 p-6 md:p-8 rounded-xl mb-8 relative overflow-hidden shadow-sm">
          ...
          <div className="relative z-10">
            ...
            <p className="text-xs text-text-secondary leading-relaxed font-sans max-w-2xl font-light">
              Explore semantic mappings across academic credentials, hackathon
              achievements, source code repositories, technical skills, and
              professional histories. Click items to browse relationships.
            </p>
          </div>
        </div>
        ...
        <Card
          hoverable={false}
          className="border-border-subtle bg-bg-panel/30 mb-8 p-6"
        >
          ...
          {pathResult && (
            <div className="p-4 bg-bg-primary/50 border border-border-subtle rounded-lg select-text overflow-x-auto">
              <span className="text-accent-cyan font-bold block mb-3 font-mono text-[10px] uppercase">
                &gt; COMPUTED_CONNECTION_PATHWAY:
              </span>
                ...
                {pathResult.map((node, idx) => {
                  ...
                  return (
                    <React.Fragment key={node.id}>
                      ...
                      >
                        <nodeConf.icon
                          className={`w-3.5 h-3.5 ${getIconColorClass(nodeConf.color)}`}
                        />
                        <span className="text-text-primary font-semibold font-mono">
                          {node.label}
                        </span>
                        <Badge
                          color={nodeConf.color}
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2 min-w-0 pr-2">
                          <nodeConf.icon
                            className={`w-3.5 h-3.5 flex-shrink-0 ${getIconColorClass(nodeConf.color)}`}
                          />
                        ...
                        <Badge
                          color={nodeConf.color}
                          className="text-[8px] font-mono leading-none flex-shrink-0 uppercase"
                        >
                          {node.type}
                        </Badge>
                      </button>
          ...
          <div className="lg:col-span-2 space-y-6">
            {selectedNode ? (
              <>
                ...
                <Card
                  hoverable={false}
                  className="border-border-subtle bg-bg-panel/30 p-6 relative overflow-hidden select-text"
                >
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-4 border-b border-border-subtle/50 pb-4 mb-4 select-none">
                    <div className="flex items-center gap-3">
                      {React.createElement(
                        getTypeBadgeStyles(selectedNode.type).icon,
                        {
                    ...
                    <Badge
                      color={getTypeBadgeStyles(selectedNode.type).color}
                      className="text-[10px] uppercase font-mono px-3 py-1"
                    >
                      {selectedNode.type}
                    </Badge>
                  </div>
                  ...
                  <div className="space-y-4 font-sans text-xs sm:text-sm">
                    {selectedNode.properties.description && (
                      <div>
                        <span className="font-mono text-[10px] text-accent-cyan uppercase block mb-1 select-none">
                          &gt; DESCRIPTION
                        </span>
                        ...
                        ([key, val]) => {
                          ...
                          if (key === 'classifications' && Array.isArray(val)) {
                            return (
                              <div
                                key={key}
                                className="bg-bg-primary/30 border border-border-subtle/50 p-2.5 rounded-lg col-span-1 sm:col-span-2"
                              >
                                <span className="text-text-secondary block text-[9px] uppercase select-none mb-1.5 font-bold">
                                  Classifications:
                                </span>
                                <div className="flex flex-wrap gap-1">
                                  {val.map((cat: string) => (
                                    <Badge
                                      ...
                                      color="purple"
                                      variant="solid"
                                      className="text-[9px] py-0.5 px-1.5 uppercase font-bold tracking-wider"
                                    >
                                      {cat}
                                    </Badge>
                                  ))}
                          ...
                          if (key === 'url' && typeof val === 'string') {
                            return (
                              <div
                                key={key}
                                className="bg-bg-primary/30 border border-border-subtle/50 p-2.5 rounded-lg"
                              >
                                <span className="text-text-secondary block text-[9px] uppercase select-none mb-0.5">
                                  SOURCE_LINK:
                                </span>
                                <a
                                  href={val}
                                  ...
                                  className="text-accent-cyan hover:underline flex items-center gap-1 font-semibold truncate"
                                >
                                ...
                                >
                                  <div className="flex items-center justify-between w-full mb-1">
                                    <div className="flex items-center gap-1.5 min-w-0 pr-1">
                                      <typeStyle.icon
                                        className={`w-3.5 h-3.5 flex-shrink-0 ${getIconColorClass(typeStyle.color)}`}
                                      />
                                    ...
                                    <Badge
                                      color="purple"
                                      variant="outline"
                                      className="text-[8px] font-mono px-1 uppercase tracking-tight py-0 leading-none"
                                    >
                                      {rel.type}
                                    </Badge>
                                  </div>
              ...
              <div className="border border-dashed border-border-subtle bg-bg-panel/10 p-12 text-center rounded-xl text-text-secondary/50 italic font-mono uppercase select-none">
                No active dossier loaded. Select an index item from the
                directory list.
              </div>
            )}
          </div>
