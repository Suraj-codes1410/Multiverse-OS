...
async function testQuery(query: string) {
  ...
  if (result) {
    ...
    console.log(
      `Recommended Project: ${result.recommendedProject.projectTitle} (Score: ${result.recommendedProject.score}/100)`
    );
    console.log(`\nAll Rankings:`);
    result.rankings.forEach((rank) => {
      console.log(
        `  Rank ${rank.rank}: ${rank.projectTitle} (Score: ${rank.score}/100)`
      );
      console.log(
        `    Repository: ${rank.repositoryName} | URL: ${rank.repositoryUrl}`
      );
      console.log(
        `    Technologies: ${rank.technologies.slice(0, 5).join(', ')}`
      );
