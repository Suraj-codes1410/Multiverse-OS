...
async function testOpenRouter() {
  ...
  if (!apiKey) {
    console.warn(
      '⚠️  WARNING: OPENROUTER_API_KEY environment variable is not defined. Skipping live OpenRouter call.'
    );
    console.log(
      'To run this test, set the environment variable: $env:OPENROUTER_API_KEY="your-key"'
    );
    return;
  }
  ...
  try {
    ...
    // Define prompts
    const query =
      "What uses FastAPI in Suraj's projects, and what is its complexity?";
...
`;
    ...
    if (response.usage) {
      console.log(
        `Tokens Used - Prompt: ${response.usage.promptTokens}, Completion: ${response.usage.completionTokens}, Total: ${response.usage.totalTokens}`
      );
    }
