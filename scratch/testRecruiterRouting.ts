...
async function testRecruiterQuery(
  query: string,
  expectedLog: 'RECRUITER_DIRECT_RESPONSE' | 'RECRUITER_MODEL_ROUTE'
) {
  console.log(`\n========================================`);
  ...
  try {
    ...
    if (
      logMatched &&
      (expectedLog === 'RECRUITER_DIRECT_RESPONSE'
...
async function runAllRecruiterTests() {
  ...
  const t3 = await testRecruiterQuery(
    'Which project demonstrates distributed systems?',
    'RECRUITER_DIRECT_RESPONSE'
  );
  if (!t3) allPassed = false;

  // Test 4: strongest technical skills (Expected: RECRUITER_DIRECT_RESPONSE)
  const t4 = await testRecruiterQuery(
  ...
  const t5 = await testRecruiterQuery(
  ...
  const t6 = await testRecruiterQuery(
    "Write a recruiter summary for Suraj's experience.",
    'RECRUITER_MODEL_ROUTE'
  );
