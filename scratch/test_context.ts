...
async function testOracleContextBuilder() {
  ...
  try {
    ...
    console.log(
      `  Degree: ${context.profile.education.degree} at ${context.profile.education.institution}`
    );
    ...
    context.skills.slice(0, 5).forEach((skill) => {
      ...
      console.log(
        `    Projects: ${skill.associatedProjects.join(', ') || 'None'}`
      );
      console.log(
        `    Repositories: ${skill.associatedRepositories.join(', ') || 'None'}`
      );
    });
    ...
    context.projects.slice(0, 5).forEach((project) => {
      ...
      console.log(
        `    Source: ${project.source}, Repository: ${project.associatedRepositoryName || 'None'}`
      );
    });
    ...
    context.repositories.slice(0, 5).forEach((repo) => {
      console.log(
        `  - ${repo.name} [stars: ${repo.starsCount}, forks: ${repo.forksCount}]`
      );
      console.log(
        `    Language: ${repo.language}, Topics: ${repo.topics.join(', ') || 'None'}`
      );
      console.log(
        `    Complexity Rating: ${repo.intelligence?.complexityRating || 'None'}`
      );
      console.log(
        `    Architecture Pattern: ${repo.intelligence?.architecturePattern || 'None'}`
      );
    });
    ...
    context.achievements.slice(0, 5).forEach((ach) => {
      ...
      console.log(
        `    Related Projects: ${ach.associatedProjects.join(', ') || 'None'}`
      );
    });
    ...
    context.technologyRelationships.slice(0, 5).forEach((tr) => {
      ...
      console.log(
        `    Related Techs: ${tr.relatedTechnologies.slice(0, 5).join(', ') || 'None'}`
      );
      console.log(
        `    Used in Projects: ${tr.usedInProjects.slice(0, 5).join(', ') || 'None'}`
      );
    });
    ...
    context.repositoryRelationships.slice(0, 5).forEach((rr) => {
      ...
      console.log(
        `    Skills Required: ${rr.skillsRequired.join(', ') || 'None'}`
      );
