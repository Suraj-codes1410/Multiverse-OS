...
async function runCacheTests() {
  ...
  assert(
    ttlGK === 24 * 60 * 60 * 1000,
    `GK TTL should be 24h, got ${ttlGK / 1000}s`
  );

  // Portfolio -> 1 Hour
  const ttlPort = queryCacheService.getTTL(
    'Which project demonstrates backend engineering?'
  );
  assert(
    ttlPort === 60 * 60 * 1000,
    `Portfolio TTL should be 1h, got ${ttlPort / 1000}s`
  );
  ...
  assert(
    ttlRepo === 30 * 60 * 1000,
    `Repo TTL should be 30m, got ${ttlRepo / 1000}s`
  );
  ...
  assert(
    ttlGit === 5 * 60 * 1000,
    `Git TTL should be 5m, got ${ttlGit / 1000}s`
  );
  ...
  assert(
    result2 === mockResponse,
    'Second lookup must return cached response (CACHE_HIT)'
  );
  ...
  assert(
    expiredResult === null,
    'Expired query must return null and trigger CACHE_EXPIRE log'
  );
