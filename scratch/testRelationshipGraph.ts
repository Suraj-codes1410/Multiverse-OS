...
async function testGraph() {
  ...
  const shared = RelationshipDiscoveryService.findSharedTechnologies(
    graph,
    'SAHAI',
    'ORBITAIR'
  );
  ...
  const intersect = RelationshipDiscoveryService.findRepositoriesWithAllTech(
    graph,
    ['FastAPI', 'React']
  );
  intersect.forEach((i) => {
  ...
  const withProps = relationships.filter(
    (rel) => rel.properties && rel.properties.confidence !== undefined
  );
  console.log(
    `Found ${withProps.length} relationships with confidence properties.`
  );
