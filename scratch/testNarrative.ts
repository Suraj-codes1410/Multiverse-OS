...
async function testNarrativeQuery(
  query: string,
  expectedLog: 'NARRATIVE_DIRECT_RESPONSE' | 'NARRATIVE_MODEL_ROUTE'
) {
  console.log(`\n========================================`);
  ...
  try {
    ...
    if (expectedLog === 'NARRATIVE_DIRECT_RESPONSE') {
      ...
      const evolutionLogged = logs.some((l) =>
        l.includes('EVOLUTION_ANALYSIS')
      );

      console.log(
        `Additional Checks - NARRATIVE_MODE: ${modeLogged} | JOURNEY_ANALYSIS: ${journeyLogged} | STRENGTH_ANALYSIS: ${strengthLogged} | EVOLUTION_ANALYSIS: ${evolutionLogged}`
      );
      ...
      if (logMatched && !result.directAnswerAvailable) {
        console.log(
          `✅ SUCCESS: Fallback to OpenRouter narrative synthesis matched!`
        );
        return true;
      }
    }
...
async function runAllNarrativeTests() {
  ...
  const t1 = await testNarrativeQuery(
  ...
  const t2 = await testNarrativeQuery(
  ...
  const t3 = await testNarrativeQuery(
    "What are Suraj's strengths and weaknesses?",
    'NARRATIVE_DIRECT_RESPONSE'
  );
  if (!t3) allPassed = false;

  // Test 4: Career positioning
  const t4 = await testNarrativeQuery(
  ...
  const t5 = await testNarrativeQuery(
    "Write a professional biography for Suraj's engineering career.",
