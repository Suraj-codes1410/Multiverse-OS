...
function testTechnologyExtraction() {
  ...
  MOCK_REPOSITORIES.forEach((repo) => {
    ...
    if (intelligence.technologyProfile) {
      ...
      Object.entries(intelligence.technologyProfile.categories).forEach(
        ([category, techs]) => {
    ...
    if (intelligence.architectureAnalysis) {
      ...
      console.log(
        `    Architecture Pattern: ${intelligence.architectureAnalysis.architecturePattern}`
      );
      if (intelligence.architectureAnalysis.communication) {
        console.log(
          `    Communication: ${intelligence.architectureAnalysis.communication.join(', ')}`
        );
      }
      if (intelligence.architectureAnalysis.security) {
        console.log(
          `    Security: ${intelligence.architectureAnalysis.security.join(', ')}`
        );
      }
      if (intelligence.architectureAnalysis.dataLayer) {
        console.log(
          `    Data Layer: ${intelligence.architectureAnalysis.dataLayer.join(', ')}`
        );
      }
    }
    if (intelligence.complexityAnalysis) {
      ...
      console.log(
        `    Overall Rating: ${intelligence.complexityAnalysis.overallRating}`
      );
      console.log(
        `    Total Score: ${intelligence.complexityAnalysis.totalScore} / ${intelligence.complexityAnalysis.maxTotalScore} PTS`
      );
      Object.entries(intelligence.complexityAnalysis.dimensions).forEach(
        ([key, dim]) => {
          console.log(`    Dimension: ${key.toUpperCase()}`);
          console.log(
            `      Rating: ${dim.rating} (${dim.score} / ${dim.maxScore} PTS)`
          );
