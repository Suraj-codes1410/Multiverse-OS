...
async function runSelectionTest() {
  ...
  try {
    ...
    console.log(
      `Full Context Size BEFORE Optimization: ${fullSizeChars} characters (~${fullTokens} tokens)`
    );
    ...
    for (let idx = 0; idx < queries.length; idx++) {
      ...
      const reductionPercent = Math.round(
        ((fullSizeChars - compressedSizeChars) / fullSizeChars) * 100
      );
      ...
      console.log(
        `Selected Sections: ${selected.selectedSections.join(', ') || 'None'}`
      );
      console.log(
        `Selected Projects: ${selected.projects.map((p) => p.title).join(', ') || 'None'}`
      );
      console.log(
        `Selected Skills: ${selected.skills.map((s) => s.name).join(', ') || 'None'}`
      );
      console.log(
        `Selected Repositories: ${selected.repositories.map((r) => r.name).join(', ') || 'None'}`
      );
      console.log(
        `Selected Achievements: ${selected.achievements.map((a) => a.title).join(', ') || 'None'}`
      );
      console.log(
        `Selected Milestones: ${selected.timeline.reduce((sum, t) => sum + t.milestones.length, 0)} milestones`
      );
