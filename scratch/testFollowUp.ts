...
async function testScenario(
  ...
  try {
    ...
    // 2. Resolve follow-up
    const resolution = conversationalMemoryService.resolve(
      sessionId,
      followUpQuery
    );
    ...
    console.log(
      `Resolved Entities: ${resolution.resolvedEntities.join(', ') || 'none'}`
    );
    ...
    if (
      ...
      console.log(
        `❌ FAILED: Resolved query did not contain expected keyword "${expectedKeyword}".`
      );
      passed = false;
    }
...
async function runAllScenarios() {
  console.log(
    'Starting Phase 5.2 Follow-Up Question Intelligence Scenarios...\n'
  );
  ...
  const t1 = await testScenario(
    ...
    [
      {
        query: 'Tell me about SAHAI',
        response: 'SAHAI is a Mental Health Platform.',
      },
    ],
  ...
  const t2 = await testScenario(
    ...
    [
      {
        query: 'Compare SAHAI and ORBITAIR.',
        response: 'Both are advanced systems.',
      },
    ],
  ...
  const t3 = await testScenario(
    ...
    [
      {
        query: 'Summarize oracle-sync-test.',
        response: 'It tests Oracle sync pipeline.',
      },
    ],
    'What technologies does it use?',
    'oracle-sync-test'
  ...
  const t4 = await testScenario(
    ...
    [
      {
        query: 'Which project demonstrates backend engineering?',
        response: 'ORBITAIR',
      },
    ],
    'Why?',
