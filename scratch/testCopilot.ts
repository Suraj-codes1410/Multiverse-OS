...
async function testCopilotQuery(
  query: string,
  expectedLog: 'CAREER_DIRECT_RESPONSE' | 'CAREER_MODEL_ROUTE'
) {
  console.log(`\n========================================`);
  ...
  try {
    ...
    if (expectedLog === 'CAREER_DIRECT_RESPONSE') {
      ...
      console.log(
        `Additional Checks - CAREER_MODE: ${modeLogged} | JOB_FIT_ANALYSIS: ${fitLogged} | SKILL_GAP_ANALYSIS: ${gapLogged} | PROJECT_RECOMMENDATION: ${recLogged} | PORTFOLIO_COPILOT: ${copilotLogged}`
      );

      if (
        logMatched &&
        result.directAnswerAvailable &&
        modeLogged &&
        fitLogged &&
        gapLogged &&
        recLogged &&
        ...
        console.log(
          `✅ SUCCESS: Career copilot routing & analysis checks matched!`
        );
        return true;
      }
    } else {
      if (logMatched && !result.directAnswerAvailable) {
        console.log(
          `✅ SUCCESS: Fallback to OpenRouter mentoring narrative matched!`
        );
        return true;
      }
    }
...
async function runAllCopilotTests() {
  console.log(
    'Starting Portfolio Copilot & Career Advisor Verification Tests...\n'
  );

  let allPassed = true;

  // Test 1: Resume Optimizer
  const t1 = await testCopilotQuery(
    'Which project should I put first on my resume?',
    'CAREER_DIRECT_RESPONSE'
  );
  if (!t1) allPassed = false;

  // Test 2: Skill Gaps
  const t2 = await testCopilotQuery(
    'What skills am I missing for backend engineering roles?',
    'CAREER_DIRECT_RESPONSE'
  );
  if (!t2) allPassed = false;

  // Test 3: Internship readiness
  const t3 = await testCopilotQuery(
    'Am I ready for a Java backend internship?',
    'CAREER_DIRECT_RESPONSE'
  );
  if (!t3) allPassed = false;

  // Test 4: Project recommendations
  const t4 = await testCopilotQuery(
  ...
  const t5 = await testCopilotQuery(
    'Which project should I showcase in interviews?',
    'CAREER_DIRECT_RESPONSE'
  );
  if (!t5) allPassed = false;

  // Test 6: Job Matching (Job Description)
  const t6 = await testCopilotQuery(
    'Which project best matches this job description for a Java Spring Boot engineer?',
    'CAREER_DIRECT_RESPONSE'
  );
  if (!t6) allPassed = false;

  // Test 7: Interview showcase (Distributed Systems focus)
  const t7 = await testCopilotQuery(
    'Which project should I showcase for distributed systems interviews?',
    'CAREER_DIRECT_RESPONSE'
  );
  if (!t7) allPassed = false;

  // Test 8: Project recommendations (Distributed Systems focus)
  const t8 = await testCopilotQuery(
    'What project would help me get into distributed systems?',
    'CAREER_DIRECT_RESPONSE'
  );
  if (!t8) allPassed = false;

  // Test 9: Long-form mentoring career advice fallback
  const t9 = await testCopilotQuery(
    'Provide career path mentoring for my long-form development goals.',
    'CAREER_MODEL_ROUTE'
  );
